...
<section className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 auto-rows-[160px] md:auto-rows-[250px]">
            {categories?.filter(c => c.pillar === 'stay').map(cat => (
              <Link key={cat.id} href={`/kategori/${cat.slug}`} className="relative rounded-lg md:rounded-lg overflow-hidden group cursor-pointer shadow-sm bg-slate-800">
                {renderBg(cat.slug)}
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 auto-rows-[150px] md:auto-rows-[220px]">
            {categories?.filter(c => c.pillar === 'explore').map(cat => (
              <Link key={cat.id} href={`/kategori/${cat.slug}`} className="relative rounded-lg md:rounded-lg overflow-hidden group cursor-pointer shadow-sm bg-[#1b1c1a]">
                {renderBg(cat.slug)}
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 auto-rows-[180px] md:auto-rows-[280px]">
            {categories?.filter(c => c.pillar === 'lifestyle').map(cat => (
              <Link key={cat.id} href={`/kategori/${cat.slug}`} className="relative rounded-lg md:rounded-lg overflow-hidden group cursor-pointer shadow-sm bg-[#3a352a]">
                {renderBg(cat.slug)}
