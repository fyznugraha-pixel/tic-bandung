...
export default function DestinationMap({ latitude, longitude, name }: DestinationMapProps) {
  ...
  if (!mounted) {
    return <div className="w-full h-full bg-gray-100 animate-pulse rounded-2xl flex items-center justify-center text-text-muted">Memuat peta...</div>;
  }
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden shadow-soft border border-gray-100">
      <MapContainer 
        center={[lat, lng]} 
        zoom={15} 
